...
AND t.typname in ('oid', 'lo', 'text', 'varchar', '_varchar', '_text', 'character_data')
...
do
  ...
  echo 'SELECT unlink_orphan_los()' | PGPASSWORD=$pass psql -eAt -U $username -h $hostname -p $port $database
done  
